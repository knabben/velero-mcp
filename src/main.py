#!/usr/bin/env python3
"""velero MCP server with dynamic tool loading.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="velero MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
        default="stdio",
    ...
    try:
        ...
        if transport_mode not in ["http", "stdio"]:
            raise ValueError(
                f"Invalid transport mode: {transport_mode}. Must be http or stdio"
            )
